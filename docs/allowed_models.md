# Danh sách các mô hình được phép sử dụng (Allowed Models)

Dưới đây là danh sách các mô hình AI/RAG/Embedding mà Ban tổ chức cho phép sử dụng, được trích xuất từ file `[DSC@UIT 2026] Danh sách mô hình.xlsx`:

| Tên mô hình | Đường dẫn (HuggingFace / Github) |
| :--- | :--- |
| AITeamVN/Vi-Qwen2-1.5B-RAG | https://huggingface.co/AITeamVN/Vi-Qwen2-1.5B-RAG |
| AITeamVN/Vi-Qwen2-3B-RAG | https://huggingface.co/AITeamVN/Vi-Qwen2-3B-RAG |
| AITeamVN/Vietnamese_Embedding | https://huggingface.co/AITeamVN/Vietnamese_Embedding |
| AITeamVN/Vietnamese_Reranker | https://huggingface.co/AITeamVN/Vietnamese_Reranker |
| AITeamVN/Vietnamese_Embedding_v2 | https://huggingface.co/AITeamVN/Vietnamese_Embedding_v2 |
| Alibaba-NLP/gte-multilingual-base | https://huggingface.co/Alibaba-NLP/gte-multilingual-base |
| Alibaba-NLP/gte-multilingual-reranker-base | https://huggingface.co/Alibaba-NLP/gte-multilingual-reranker-base |
| BAAI/bge-m3 | https://huggingface.co/BAAI/bge-m3 |
| BAAI/bge-reranker-base | https://huggingface.co/BAAI/bge-reranker-base |
| BAAI/bge-reranker-v2-m3 | https://huggingface.co/BAAI/bge-reranker-v2-m3 |
| bkai-foundation-models/vietnamese-bi-encoder | https://huggingface.co/bkai-foundation-models/vietnamese-bi-encoder |
| bqbbac6/vietnamese-legal-embedding | https://huggingface.co/bqbbac6/vietnamese-legal-embedding |
| cross-encoder/mmarco-mMiniLMv2-L12-H384-v1 | https://huggingface.co/cross-encoder/mmarco-mMiniLMv2-L12-H384-v1 |
| cross-encoder/ms-marco-MiniLM-L-6-v2 | https://huggingface.co/cross-encoder/ms-marco-MiniLM-L-6-v2 |
| DEK21_hcmute_embedding_v2 | https://huggingface.co/huydangg/DEK21_hcmute_embedding_v2 |
| google/embeddinggemma-300m | https://huggingface.co/google/embeddinggemma-300m |
| google/gemma-3-1b-it | https://huggingface.co/google/gemma-3-1b-it |
| HuggingFaceTB/SmolLM3-3B | https://huggingface.co/HuggingFaceTB/SmolLM3-3B |
| huydangg/DEK21_hcmute_embedding_v2 | https://huggingface.co/huydangg/DEK21_hcmute_embedding_v2 |
| infgrad/Prism-Qwen3.5-Reranker-2B | https://huggingface.co/infgrad/Prism-Qwen3.5-Reranker-2B |
| intfloat/multilingual-e5-base | https://huggingface.co/intfloat/multilingual-e5-base |
| intfloat/multilingual-e5-large | https://huggingface.co/intfloat/multilingual-e5-large |
| intfloat/multilingual-e5-large-instruct | https://huggingface.co/intfloat/multilingual-e5-large-instruct |
| intfloat/multilingual-e5-small | https://huggingface.co/intfloat/multilingual-e5-small |
| itdainb/PhoRanker | https://huggingface.co/itdainb/PhoRanker |
| jinaai/jina-reranker-v2-base-multilingual | https://huggingface.co/jinaai/jina-reranker-v2-base-multilingual |
| jinaai/jina-reranker-v3.5 | https://huggingface.co/jinaai/jina-reranker-v3.5 |
| keepitreal/vietnamese-sbert | https://huggingface.co/keepitreal/vietnamese-sbert |
| namdp-ptit/ViRanker | https://huggingface.co/namdp-ptit/ViRanker |
| nvidia/Nemotron-3-Embed-1B-BF16 | https://huggingface.co/nvidia/Nemotron-3-Embed-1B-BF16 |
| Qwen/Qwen2.5-1.5B-Instruct | https://huggingface.co/Qwen/Qwen2.5-1.5B-Instruct |
| Qwen/Qwen2.5-3B-Instruct | https://huggingface.co/Qwen/Qwen2.5-3B-Instruct |
| Qwen/Qwen2.5-3B-Instruct-GGUF | https://huggingface.co/Qwen/Qwen2.5-3B-Instruct-GGUF |
| Qwen/Qwen2.5-Math-1.5B-Instruct | https://huggingface.co/Qwen/Qwen2.5-Math-1.5B-Instruct |
| Qwen/Qwen3-1.7B | https://huggingface.co/Qwen/Qwen3-1.7B |
| Qwen/Qwen3-4B | https://huggingface.co/Qwen/Qwen3-4B |
| Qwen/Qwen3-4B-Instruct-2507 | https://huggingface.co/Qwen/Qwen3-4B-Instruct-2507 |
| Qwen/Qwen3-Embedding-0.5B | https://huggingface.co/Qwen/Qwen3-Embedding-0.5B |
| Qwen/Qwen3-Reranker-0.5B | https://huggingface.co/Qwen/Qwen3-Reranker-0.5B |
| Qwen/Qwen3.5-0.5B | https://huggingface.co/Qwen/Qwen3.5-0.5B |
| Qwen/Qwen3.5-2B | https://huggingface.co/Qwen/Qwen3.5-2B |
| Qwen/Qwen3.5-4B | https://huggingface.co/Qwen/Qwen3.5-4B |
| Qwen3.5-2B-GGUF | https://huggingface.co/unsloth/Qwen3.5-2B-GGUF |
| Qwopus3.5-2B-v3 | https://huggingface.co/Jackrong/Qwopus3.5-2B-v3 |
| thangvip/qwen3-1.7b-vietnamese-legal-grpo-phase-2 | https://huggingface.co/thangvip/qwen3-1.7b-vietnamese-legal-grpo-phase-2 |
| thanhtantran/Vietnamese_Reranker | https://huggingface.co/thanhtantran/Vietnamese_Reranker |
| underthesea | https://github.com/undertheseanlp/underthesea |
| VietAI/vit5-base | https://huggingface.co/VietAI/vit5-base |
| VietAI/vit5-large | https://huggingface.co/VietAI/vit5-large |
| vietlegal-harrier-0.6b | https://huggingface.co/mainguyen9/vietlegal-harrier-0.6b |
| ViLegalBERT | https://huggingface.co/mtphuc149/ViLegalBERT |
| ViLegalQwen2.5-1.5B-Base | https://huggingface.co/mtphuc149/ViLegalQwen2.5-1.5B-Base |
| ViLegalQwen3-1.7B-Base | https://huggingface.co/mtphuc149/ViLegalQwen3-1.7B-Base |
| vilm/vinallama-2.7b-chat | https://huggingface.co/vilm/vinallama-2.7b-chat |
| vinai/phobert-base-v2 | https://huggingface.co/vinai/phobert-base-v2 |
| vinai/phobert-base | https://huggingface.co/vinai/phobert-base |
| vinai/PhoGPT-4B-Chat | https://huggingface.co/vinai/PhoGPT-4B-Chat |
| VnCoreNLP | https://github.com/vncorenlp/VnCoreNLP |